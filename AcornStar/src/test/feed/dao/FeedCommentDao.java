...
import test.feed.dto.FeedCommentDto;
import test.util.DbcpBean;

public class FeedCommentDao {
	private static FeedCommentDao dao;
	/*
	 *  [ static 초기화 블럭 ]
	...
	//댓글 내용을 수정하는 메소드
	public boolean update(FeedCommentDto dto) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		int flag = 0;
		try {
	...
	//댓글 목록을 리턴하는 메소드
	public List<FeedCommentDto> getList(FeedCommentDto dto2){
		List<FeedCommentDto> list=new ArrayList<>();
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
	...
	public int getSequence() {
		...
		try {
			...
			conn = new DbcpBean().getConn();
			//실행할 sql 문 작성
			String sql = "SELECT Feed_comment_seq.NEXTVAL AS seq"
					+ " FROM DUAL";
			//PreparedStatement 객체의 참조값 얻어오기
			pstmt = conn.prepareStatement(sql);
			//? 에 바인딩할 내용이 있으면 여기서 바인딩
	...
	//댓글 추가
	public boolean insert(FeedCommentDto dto) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		int flag = 0;
		try {
			conn = new DbcpBean().getConn();
			//실행할 sql 문 작성
			String sql = "INSERT INTO Feed_comment"
