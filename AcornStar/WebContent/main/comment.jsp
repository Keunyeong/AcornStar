...
<div class="container">
	...
	<form class="comment-form insert-form" action="comment_insert.jsp" method="post">
		...
		<input id="target_id" type="hidden" name="target_id" value="<%=dto.getWriter()%>"/>
...
<script>
	document.querySelector("#commentBtn").addEventListener("click",function(e){
		e.preventDefault();
		let target = document.querySelector("#target_id").value;
		let comment = document.querySelector("#comment_content").value;
		ajaxPromise("comment_insert.jsp", "post", {ref_group:<%=num%>,target_id:target,comment:comment})
