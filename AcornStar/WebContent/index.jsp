	      ...
	      <div class="modal-body">
	        <form action="<%=request.getContextPath() %>/user/signup.jsp" method="post" id="myForm" class="row">
		      ...
		      <div>
		         ...
		         <input class="form-control" type="password" name="pwd" id="pwd1"/>
...
<script>
   ...
   document.querySelector("#id1").addEventListener("input", function(){
      ...
      document.querySelector("#id1").classList.remove("is-valid");
      ...
      if(!reg_id.test(inputId)){
         ...
         document.querySelector("#id1").classList.add("is-invalid");
      ...
      .then(function(data){
         ...
         if(data.isExist){//만일 존재한다면
            ...
            isIdValid=true;
            document.querySelector("#id1").classList.add("is-valid");
         }
      });
   });
   ...
   //비밀 번호를 확인 하는 함수 
   function checkPwd(){
      document.querySelector("#pwd1").classList.remove("is-valid");
      ...
      if(pwd != pwd2){//비밀번호와 비밀번호 확인란이 다르면
         ...
         isPwdValid=true;
         document.querySelector("#pwd1").classList.add("is-valid");
      }
   }
   
   //비밀번호 입력란에 input 이벤트가 일어 났을때 실행할 함수 등록
   document.querySelector("#pwd1").addEventListener("input", checkPwd);
