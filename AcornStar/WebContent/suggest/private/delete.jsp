...
<%
	...
	String writer=SuggestDao.getInstance().getData(num).getWriter();
	String id=(String)session.getAttribute("id");
	//만일 글 작성자와 로그인된 아이디가 다르면
	if(!writer.equals(id)){
		//금지된 요청이라고 응답하고 
		response.sendError(HttpServletResponse.SC_FORBIDDEN, "타인의 글을 삭제 할 수 없습니다.");
		return; // 여기서 메소드 종료 
	}
	//3. 같으면 DB 에서 삭제하고 응답 
	boolean isSuccess=SuggestDao.getInstance().delete(num);
...
<head>
<meta charset="UTF-8">
<title>/info/private/delete.jsp</title>
</head>
<body>
	<%if(isSuccess){%>
		<script>
